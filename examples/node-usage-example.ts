...
async function demonstrateUsage() {
  ...
  try {
    ...
    console.log(`   ❌ Auto deployer failed: ${(error as Error).message}\n`);
  }

  // Example 2: Explicit API-only mode (Val.town style)
  console.log('2. API-only mode (Val.town compatible):');
  try {
    const valTownDeployer = createValTownDeployer({
    ...
    console.log(`   ❌ Val.town mode failed: ${(error as Error).message}\n`);
  }
  ...
  try {
    ...
    console.log(`   ❌ Local mode failed: ${(error as Error).message}\n`);
  }
  ...
  if (process.env.GITHUB_TOKEN && process.env.ASTRO_REPO_URL) {
    try {
      await testGitHubApiAccess(
        ...
        process.env.ASTRO_REPO_URL
      );
      console.log('   ✅ GitHub API access verified\n');
    } catch (error) {
      console.log(`   ❌ GitHub API test failed: ${(error as Error).message}\n`);
    }
  } else {
...
*This PR was generated automatically by EvntHndlr*
    ...
    console.log(`   ❌ Custom deployment failed: ${(error as Error).message}\n`);
  }
...
export { demonstrateUsage, cliExample, environmentSetup };